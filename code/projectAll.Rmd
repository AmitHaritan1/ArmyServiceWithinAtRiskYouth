...
df_table <- df_pred %>%
  mutate(
    army      = if_else(T3army == 1, "Served", "not Served"),
    army_pred = if_else(.pred_1 > cutoff_prob, "child labelled as Served", "child labelled as not Served")
    ) %>%
  count(army_pred, army) %>%
  pivot_wider(names_from = army, values_from = n)
...
TP <- df_mat[1, 1] # not served, not predicted
FP <- df_mat[1, 2] # not served, predicted
FN <- df_mat[2, 1] # served, not predicted
TN <- df_mat[2, 2] # served, predicted
...
TP <- confusion_matrix[1, 1] # not served, not predicted
FP <- confusion_matrix[1, 2] # not served, predicted
FN <- confusion_matrix[2, 1] # served, not predicted
TN <- confusion_matrix[2, 2] # served, predicted
